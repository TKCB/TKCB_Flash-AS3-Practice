...
package cc.tkcb.draw
{
	...
	public class Polygon
	{
		...
		/**
		 * 绘制多边形
		 * @param vertex 顶点（边）的个数
		 * @param sideLength 边长度
		 ...
		 **/
		public static function drawPolygon ( vertex:int = 6, sideLength:int = 50, lineThickness:Number = 1, lineColor:uint = 0x000000, 
										  lineAlpha:Number = 1, fillColor:uint = 0x000000, fillAlpha:Number = 1 ) : Shape
		{
			if ( vertex >= 2 )
			{
				...
				shape.graphics.moveTo( sideLength, 0 );
				
				var angle : Number;
				var i:int, len:int = vertex;
				for ( i = 1; i < len; i++ )
				{
					// 当前角度
					angle = Math.PI * 2 / vertex * i;
					
					// 点的坐标（通过角度与半径计算每一个顶点的坐标）
					shape.graphics.lineTo( Math.cos(angle) * sideLength, Math.sin(angle) * sideLength );
				}
