...
package cc.tkcb.display.psd
{
	...
	public class PSDLayer
	{
		...
		public var layerID: uint = 0;
		public var numChannels: int = 0;
		public var channelsInfo_arr: Array = null;
		public var blendModeKey: String = null;
		public var blendMode: String = null;
		public var alpha: Number = 0;
		public var maskBounds: Rectangle = null;
		public var clippingApplied: Boolean = false;;
		public var isLocked: Boolean = false;;
		public var isVisible: Boolean = false;;
		public var pixelDataIrrelevant: Boolean = false;;
		public var nameUNI: String = null; //layer unicode name		
		public var filters_arr: Array = null; //filters array	
		...
		public function dispose(): void
		{
			...
			if (channelsInfo_arr)
			{
				channelsInfo_arr.length = 0;
				channelsInfo_arr = null;
			}
			if (filters_arr)
			{
		...
		private function readLayerBasicInfo(): void
		{
			...
			*/
			...
			if (sig != "8BIM")
				throw new Error("Invalid Blend mode signature: " + sig);
			...
			*/
			...
			//irrelevant
			if ((flags & 3) != 0)
				pixelDataIrrelevant = (flags & 4) != 0; //543
			...
			const extraSize: uint = fileData.readUnsignedInt(); //561
			...
			while (fileData.position - pos < extraSize)
			{
				...
				//check signature
				if (sig != "8BIM")
					throw new Error("layer information signature error");
				...
				*/
				...
				switch (tag)
				{
					//------------------------------------------------------------- get layer ID
					case "lyid":
						layerID = fileData.readInt();
						break;

						//------------------------------------------------------------- get layer divider section
					case "lsct":
						readLayerSectionDevider();
						break;

						//------------------------------------------------------------- get layer unicode name
					case "luni":
						nameUNI = fileData.readUTFBytes(4);
						break;
		...
		private function parseGlow(fileData: ByteArray, inner: Boolean = false): void
		{
			...
			*/
			...
			{
				if (inner)
					var invert: Boolean = fileData.readBoolean();
		...
		private function readLayerSectionDevider(): void
		{
			...
			{
				case 0:
					type = LayerType_NORMAL;
					break;
				case 1:
					type = LayerType_FOLDER_OPEN;
					break;
				case 2:
					type = LayerType_FOLDER_CLOSED;
					break;
				case 3:
					type = LayerType_HIDDEN;
					break;
			}
		}
		...
		private function parseLayerMaskData(stream: ByteArray): void
		{
			...
			*/
			const maskSize: uint = stream.readUnsignedInt();
			...
			if (maskSize > 0)
			{
				...
				if (maskSize == 20)
				{
					var maskPadding: int = stream.readUnsignedShort(); // 723 (readShortInt)
				}
				else
				{
