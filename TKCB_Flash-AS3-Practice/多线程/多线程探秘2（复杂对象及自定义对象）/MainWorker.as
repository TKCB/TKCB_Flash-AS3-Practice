package
{
	...
	public class MainWorker extends Sprite
	{
		...
		private function addedToStage ( eve : Event ) : void
		{
			...
			// 给自定义对象注册别名，用于反序列化后类型不变
			registerClassAlias( "com.tkcb.CustomObject", CustomObject  );
		...
		private function childrenToMainChannelMessage ( eve : Event ) : void
		{
			...
			{
				childrenToMainTF.appendText( "返回的数组：" + obj.toString() + "\n" );
			}
			else if ( obj is CustomObject )
			{
				childrenToMainTF.appendText( "自定义对象 CustomObject  attribute ：" + obj.attribute );
			}
		}
		...
		private function mainToChildrenChannelSend () : void
		{
			...
			mainToChildrenChannel.send( arr );
			mainToChildrenChannel.send( customObject );
			
			mainToChildrenTF.appendText( str + "\n" );
			mainToChildrenTF.appendText( "发送的数组：" + arr.toString() + "\n" );
			mainToChildrenTF.appendText( "自定义对象 CustomObject  attribute ：" + customObject.attribute );
		}
