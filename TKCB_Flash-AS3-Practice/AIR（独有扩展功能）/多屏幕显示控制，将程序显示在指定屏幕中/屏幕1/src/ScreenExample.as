package 
{
	...
	/**
	 * ScreenExample 类，将程序显示在指定屏幕中
	 * @author TKCB
	 * QQ 2414268040
	 */
	public class ScreenExample extends Sprite
	{
		private var locationX:uint;		// X轴位置
		private var locationY:uint;		// Y轴位置
		...
		function ScreenExample()
		{
			...
			stage.scaleMode = StageScaleMode.EXACT_FIT;		// 指定缩放模式，拉伸缩放模式
		...
		private function setScreen():void
		{
			...
			if (Screen.screens.length > 1)
			{
				if(locationX > 1)
				{
					for(var i:uint = 1; i < locationX; i++)
					{
						moveRight();
						
						stage.align = StageAlign.TOP_LEFT;				// 指定舞台在 Flash Player 或浏览器中的对齐方式，顶对齐 左对齐
						stage.scaleMode = StageScaleMode.NO_SCALE;		// 指定缩放模式，拉伸缩放模式
		...
		{
			var currentScreen:Screen = getCurrentScreen();		// 获取当前主屏幕对象
